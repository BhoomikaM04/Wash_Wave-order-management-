...
if (isset($_POST['update_status'])) {
    ...
    if ($stmt->execute()) {
        header("Location: admin-orders-ready.php?success=Laundry status updated successfully!");
        exit();
    }
}
...
if (isset($_GET['action']) && $_GET['action'] == 'confirm_received' && isset($_GET['order_id'])) {
    ...
    if ($stmt->execute()) {
        header("Location: admin-orders-ready.php?success=Payment marked as Paid in database!");
        exit();
    }
}
?>
...
<head>
    ...
    <title>Orders Ready Panel - WashWave</title>
    ...
    <style>
        ...
        .pm-upi { color: #0d6efd; }
        .pm-cod { color: #4f46e5; }
        .form-select-sm { font-size: 13px; border-radius: 6px; }
    </style>
</head>
<body>
...
<div class="container-fluid px-4 py-5">
    <div class="card border-0 shadow-sm p-4">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fw-bold text-dark m-0"><i class="fas fa-clock me-2 text-info"></i> WashWave Orders Ready Registry</h4>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle border">
                ...
                <tbody>
                    <?php
                    // FILTER: Strictly queries records marked as 'Ready'
                    $query = "SELECT * FROM orders WHERE status = 'Ready' ORDER BY created_at DESC";
                    ...
                    if ($result && $result->num_rows > 0) {
                        while($row = $result->fetch_assoc()) {
                            ...
                            <tr>
                                ...
                                <td><span class="badge-status status-ready">Ready</span></td>
                                <td>
                                    <form action="admin-orders-ready.php" method="POST" class="d-flex gap-1 align-items-center">
                                        <input type="hidden" name="order_id" value="<?php echo $row['id']; ?>">
                                        <select name="status" class="form-select form-select-sm" style="width: 120px;">
                                            <?php
                                            // CHANGED: Swapped 'Completed' out for 'Delivered' inside selection options array
                                            $statuses = ['Pending', 'Processing', 'Ready', 'Delivered', 'Cancelled'];
                                            foreach($statuses as $st) {
                                                $selected = ($db_status == $st || ($st == 'Delivered' && $db_status == 'Completed')) ? 'selected' : '';
                                ...
                                <td class="text-center">
                                    ...
                                    <?php else: ?>
                                        <a href="admin-orders-ready.php?action=confirm_received&order_id=<?php echo $row['id']; ?>" class="btn btn-sm btn-success fw-bold py-1 px-2" style="font-size:12px;">Confirm Cash Received</a>
                                    <?php endif; ?>
                                </td>
                                <td>
                        ...
                        echo "<tr><td colspan='11' class='text-center py-4 text-muted'>No ready orders waiting in records.</td></tr>";
                    }
                    ?>
                </tbody>
            </table>
